...
7 有什么办法使不让人对类的变量在外部进行一个赋值改变了 或者是读取操作 原则上我们是需要通过方法来修改对应的类变量与是列变量的
...
10 Student.__socre 也就是动态添加的，存在于__dict__列表中 所以通过动态访问也是能访问到列表的 python的动态性
11 我们可以通过单下划线类名称多双下划线能读取这样的形式


没有什么是不能访问的:
    1 私有变量在类的外部进行改变为啥不会报错了 但是在调用私有方法的时候就会报错这是为啥了
