...
if __name__ == "__main__":
    user = Student("建国长城")
    """
    1 通过__dict__ 去查询我们数据结构的内部属性 这里的时候我们使用的Cpython来编写的 dict其实性能是十分的搞的
    
    这样打印__dict__ 那么他就会自动去找到我们
    user这个实例的属性，也就是叫为实例属性
    
    """
    print(user.__dict__)   # 这里也就叫实例属性
    print('-------------------------')
    """
    动态操作我们user对象的一个过程
    ...
    print('-------------Person-----------------')
    """
    1 类比我们的实例更加的丰富所以的类的属性值要比对象的多
    2 类中对象的数据结构
    """
    print(Person.__dict__)
    print('------------user.name------------------')
    # 这里就会向上查找 也就是通过继承的关系去查找的
    # 这个-name-也就是Person对象空间的属性
    print(user.name)
