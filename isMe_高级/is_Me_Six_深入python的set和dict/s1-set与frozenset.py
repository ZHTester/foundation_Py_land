"""
1 set 可变集合(无序的集合，不重复的集合) 和 fronzenset 不可变集合
2 set(iterable) -> new set object  这里就是一个可迭代的对象 也就是集合 - set
3 集合中常见的运算 -  & /  也就是数学运算  实际上这里也是实现了一堆的魔法函数
4 dict 也是常见的使用数据结构类型
5 set的性能是十分的搞 他们背后都是实现的hash的计算方式，所以set在我们去重复的时候是十分的有效的
6 如何判断一个元素是否在set当中，我们可以是用
                                            if  'c' in resset:  #set 是实现了     def __contains__(self, *args, **kwargs):魔法函数
                                                print(c)
                                            else:
                                                print("我没有在这个数组当中")
...
# ----- frozenset 不可变的数据类型
# s = frozenset("adbde")
# # s.add('c') 这里是没有add方法的所以说 frozenset 一旦设置完成后就无法修改了  他就可以作为dict的key
# print(s)
